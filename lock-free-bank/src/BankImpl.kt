...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
            ...
            val firstAcquired = acquire(minIndex, this)
            val secondAcquired = acquire(maxIndex, this)
            if (firstAcquired == null || secondAcquired == null) {
                release(minIndex, this)
                release(maxIndex, this)
                return
            }

            val from = if (fromIndex < toIndex) {
                firstAcquired
            } else {
                secondAcquired
            }
            val to = if (fromIndex < toIndex) {
                secondAcquired
            } else {
                firstAcquired
            }
